...
def extract_img_middle_slices(img:np.ndarray) -> Tuple[np.array]:
...
def add_mask(img_warped:np.ndarray, template_seg:np.ndarray, seg_reconst:np.ndarray) -> np.ndarray:
...
def disp_warped_img(img1:np.ndarray, img1_recons:np.ndarray, img2:np.ndarray) -> np.ndarray:    
...
def extract_flow_middle_slices(flow:np.ndarray) -> List[Tuple[np.ndarray]]:
    return [extract_img_middle_slices(flow[i,:,:,:]) for i in range(3)]
    
def disp_training_fig(img1:np.ndarray, img2:np.ndarray, flow:np.ndarray) -> np.ndarray:
...
def get_2d_flow_sections(flow:np.ndarray) -> Tuple[np.ndarray]:
...
def disp_flow_colors(flow:np.ndarray) -> np.ndarray:
    slice_x_flow, slice_y_flow, slice_z_flow = get_2d_flow_sections(flow)
...
def _add_flow_contour_arrows(image:np.ndarray, contours:np.ndarray, slice_flow:np.ndarray, arrow_scale_factor:int, slice_name:str, equal_arrow_length:bool=False, emphesize:float=0., arrow_color=None, circle_color=None, thickness=1, paper_vis_config:bool=False):#bool=False) -> np.ndarray:
    if not(paper_vis_config):
        arrow_color, circle_color = ((0, 0, 0), (1, 1, 1)) if not emphesize else ((1, 0, 0.1), (1, 0.1, 0))
    if paper_vis_config:
        num_arrows_scale_down_factor = 7
        contours = contours[::num_arrows_scale_down_factor]
    for contour in contours:
        ...
        l = max(np.linalg.norm((start-end),2),1)
        image = cv2.arrowedLine(image, (start[0], start[1]), (end[0], end[1]), color=arrow_color, thickness=thickness, line_type=cv2.LINE_4, tipLength=5./l)
        if not(paper_vis_config):
            image = cv2.circle(image, (end[0], end[1]), radius=1, color=circle_color, thickness=-1)

    return image

def _get_arrow_start_end_coords(contour:np.ndarray, slice_flow:np.ndarray, arrow_scale_factor:int, slice_name:str, equal_arrow_length:bool) -> Tuple[np.ndarray, np.ndarray]:
    start = contour[0]
